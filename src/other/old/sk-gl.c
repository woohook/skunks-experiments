...
REALN  zfog,zmax,
       vfov; /*zfog,zmax - dist. where fog starts, max. visible dist.*/
lightpr light;
...
/*SDL initialized*/
   ...
   gluPerspective(vfov,(GLfloat)GLWIDTH/(GLfloat)GLHEIGHT,0.25f,zmax); /*0.25, zmax - clipping planes*/
#else
   vfov=114.6*atan(tan(0.008727*FOV)*GLHEIGHT*(GLfloat)WIDTHFACTOR/GLWIDTH);
   gluPerspective(vfov,(GLfloat)GLWIDTH/(GLfloat)GLHEIGHT/(GLfloat)WIDTHFACTOR,0.25f,zmax); /*0.25, zmax - clipping planes*/
#endif
...
backcol.glred=1+dmode; backcol.glgreen=speed; /*no text on screen with OpenGL*/
/*
switch(dmode){
