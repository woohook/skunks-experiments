...
{
  struct _camera* cam = malloc(sizeof(struct _camera));
  cam->view_transform = malloc(sizeof(struct _matrix));
  matrix_identity(cam->view_transform);
  cam->object = 0;
...
{
  struct _matrix *camera = camera_get_view_transform(cam);
  struct _matrix* object = camera_get_focus(cam);
  ...
  h=camera->vy[3]-camera->vy[0];
  d=camera->vz[3]-camera->vz[0];
  a=y0*h+z0*d;
